...
export function useAssetsPage(projectId: string | undefined, q: AssetListQuery) {
  return useQuery({
    queryKey: ["assets-page", projectId ?? "", q.pool ?? "verified", q.rootDomain ?? "", q.q ?? "", q.liveOnly ? "1" : "0", q.monitorNew ?? "all", String(q.page ?? 1), String(q.pageSize ?? 50), q.sortBy ?? "", q.sortDir ?? ""],
